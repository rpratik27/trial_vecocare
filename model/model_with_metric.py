...
if __name__ == "__main__":
    ...
    time_diffs     = load_pkl("time_diffs.pkl")
    patient_labels  = load_pkl("/Users/pratikranjan/Desktop/vecocare_v2.0/patient_labels_multihot.pkl")
# ---------------------- Additional Code: Evaluation Metrics ----------------------
# This block does not change any of your original code;
# it simply adds the computation of mi-AUROC, ma-AUROC, mi-AUPRC, and ma-AUPRC.
d = 128  # Define your base embedding dimension here

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Load models from AMLM training phase
base_encoder = BaseEncoder(len(code_map), len(vocab), d).to(device)
transformer_notes = TransformerEncoder(d).to(device)
transformer_visits = TimeAwareTransformer(2*d).to(device)
base_encoder.load_state_dict(torch.load("trained_base_encoder_amlm.pth"))
transformer_notes.load_state_dict(torch.load("trained_transformer_notes_amlm.pth"))
transformer_visits.load_state_dict(torch.load("trained_transformer_visits_amlm.pth"))
base_encoder.eval()
transformer_notes.eval()
transformer_visits.eval()

# For classification, assume a trained classification head has been saved.
sample_label = next(iter(patient_labels.values()))
label_dim = len(sample_label) if isinstance(sample_label, (list, np.ndarray)) else sample_label.shape[0]
classification_layer = torch.nn.Linear(4*d, label_dim).to(device)
try:
    classification_layer.load_state_dict(torch.load("trained_classification_layer.pth"))
except:
    print("No trained classification head found. Metrics may not reflect trained performance.")
...
for pid in patient_labels:
    ...
    note_data = patient_notes.get(pid)
    if note_data is None:
        note_tensor = torch.zeros((1, len(vocab)), dtype=torch.float32, device=device)
    else:
        note_tensor = torch.tensor(note_data, dtype=torch.float32, device=device)
    with torch.no_grad():
        base_v, base_n = base_encoder(icd_tensor, note_tensor)
        ...
        tv = transformer_visits(base_v.unsqueeze(0), t_tensor.unsqueeze(0)).squeeze(0)
        tv = tv[:, :d]
        # Compute attention-based CLS tokens
        H = tn.unsqueeze(0)  # (1, T, d)
        U = tv.unsqueeze(0)  # (1, T, d)
        query = torch.mean(H, dim=1, keepdim=True)
        attention_weights = F.softmax(torch.matmul(query, H.transpose(-1,-2)), dim=-1)
        H_cls = torch.matmul(attention_weights, H).squeeze(0)
        query_U = torch.mean(U, dim=1, keepdim=True)
        attention_weights_U = F.softmax(torch.matmul(query_U, U.transpose(-1,-2)), dim=-1)
        U_cls = torch.matmul(attention_weights_U, U).squeeze(0)
        # Use an empty memory bank for evaluation.
        r = torch.cat([H_cls, U_cls, torch.zeros_like(H_cls), torch.zeros_like(H_cls)], dim=-1)
        logits = classification_layer(r)
        Y_pred = torch.sigmoid(logits).squeeze(0).cpu().numpy()
    y_score_all.append(Y_pred)
    y_true_all.append(np.array(patient_labels[pid]))

y_true_all = np.stack(y_true_all, axis=0)
y_score_all = np.stack(y_score_all, axis=0)

import numpy as np

# Suppose y_true_all and y_score_all both have shape (N, L).
# We'll remove any label j that has no positives in y_true_all.
mask_has_positives = (y_true_all.sum(axis=0) > 0)
y_true_filtered = y_true_all[:, mask_has_positives]
y_score_filtered = y_score_all[:, mask_has_positives]

# Then compute your metrics on the filtered arrays:
mi_auroc = roc_auc_score(y_true_filtered, y_score_filtered, average='micro')
ma_auroc = roc_auc_score(y_true_filtered, y_score_filtered, average='macro')
mi_auprc = average_precision_score(y_true_filtered, y_score_filtered, average='micro')
ma_auprc = average_precision_score(y_true_filtered, y_score_filtered, average='macro')


# mi_auroc = roc_auc_score(y_true_all, y_score_all, average='micro')
# ma_auroc = roc_auc_score(y_true_all, y_score_all, average='macro')
# mi_auprc = average_precision_score(y_true_all, y_score_all, average='micro')
# ma_auprc = average_precision_score(y_true_all, y_score_all, average='macro')

print("Evaluation Metrics:")
# print("mi-AUROC: {:.4f}".format(mi_auroc))
# print("ma-AUROC: {:.4f}".format(ma_auroc))
print("mi-AUPRC: {:.4f}".format(mi_auprc))
print("ma-AUPRC: {:.4f}".format(ma_auprc))
